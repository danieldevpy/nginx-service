...
class NginxRepositoryDocker(NginxRepository):
    """
    ...
    def enable_server(self, server_name: str) -> None:
        ...
        if not enabled_path.exists():
            self._run(["ln", "-s", str(available_path), str(enabled_path)])
    ...
    def get_status(self) -> str:
        """
        Retorna o status do Nginx dentro do container
        simulando a saída do systemctl/service.
        """
        try:
            # Chama o service para pegar o status
            result = subprocess.run(
